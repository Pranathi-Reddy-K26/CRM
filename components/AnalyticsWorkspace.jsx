...
import { ActivityMix, AnalyticsHeader, AnalyticsSummary, LargestOpportunities, PipelineMomentum, PipelineStageFlow, TaskHealth, TopAccounts } from "@/components/analytics/AnalyticsSections";
...
export default function AnalyticsWorkspace({ tasks, onViewCompany, onViewDeal, onSelectActivity }) {
  const model = useMemo(() => makeModel(tasks), [tasks]);
  const [view, setView] = useState("universe");
  const [leavingView, setLeavingView] = useState(null);
  ...
  useEffect(() => {
    ...
    const timeout = window.setTimeout(() => setLeavingView(null), 340);
    return () => window.clearTimeout(timeout);
  }, [leavingView, view]);

  const select = item => {
    const isSame = selection?.type === item?.type && selection?.id === item?.id;
    if (!item || isSame) {
      setSelection(null);
      setFocused(null);
      return;
  ...
  const selectStageFromFlow = stage => { changeView("universe"); toggleStage(stage); };
  const renderView = which => which === "universe"
    ? <PipelineUniverse model={model} focused={focused} selection={selection} selectedStage={selectedStage} setFocused={setFocused} onSelect={select} onToggleStage={toggleStage} animateEntrance={animateUniverse} />
    : which === "funnel" ? <Funnel model={model} select={select} /> : <Trend model={model} />;
  ...
  return <div className="analytics-workspace">
    ...
    <section className="au-hero" aria-labelledby="au-universe-title">
      ...
      <div className="au-view-content">
        {leavingView && <div key={`leaving-${leavingView}`} className="au-view-panel is-leaving" aria-hidden="true" inert>{renderView(leavingView)}</div>}
        <div key={view} id="au-view-panel" className="au-view-panel is-current" role="tabpanel" aria-labelledby={`au-tab-${view}`}>{renderView(view)}</div>
      </div>
    </section>
    <div className="av-dashboard">
      <div className="av-duo av-duo-primary">
        <PipelineMomentum model={model} onFocus={setFocused} />
        <ActivityMix tasks={tasks} onSelectType={onSelectActivity} />
      </div>
      <div className="av-duo av-duo-secondary">
        <PipelineStageFlow model={model} onFocus={setFocused} onSelectStage={selectStageFromFlow} selectedStage={selectedStage} />
        <TaskHealth model={model} />
      </div>
      <TopAccounts model={model} highlightedCompanyId={(selection || focused)?.type === "company" ? (selection || focused).id : null} onFocus={setFocused} onSelect={selectFromList} />
      <LargestOpportunities model={model} highlightedDealId={(selection || focused)?.type === "deal" ? (selection || focused).id : null} onFocus={setFocused} onSelect={selectFromList} />
    </div>
